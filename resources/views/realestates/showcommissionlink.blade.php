...
@section('content')
            ...
            <div class="tableFixHead" style="padding:0px;">
               <table id="mytable" class="table table-bordered table-hover tbl_transferlist" style="table-layout:fixed;">
                   <thead style="text-align:center;" class="kh16">
                        ...
                        <th style="width:120px;">MapID</th>
                         <th style="width:100px;">TCode</th>
                   ...
                   <tbody id="body_transaction">
                        @foreach ($transfers as $key => $item)
                            <tr class="kh16-b">
                                ...
                                <td style="color:gray">{{ $item->map_id }}</td>
                                  <td style="text-align:center;">{{  $item->trancode  }}</td>
                                ...
                                <td>{{ $item->tranname . '(' . $item->sendername . ')' }}</td>
                                <td style="text-align:right;color:blue;">{{ phpformatnumber($item->amount)}}</td>
                               <td>{{ $item->currency->shortcut }}</td>


                                <td>{{ $item->ref_group_id }}</td>
                               <td>
                                    <a href="" data-id="{{$item->id}}" data-cid="{{$item->parrent_id}}" data-amount="{{ $item->amount }}" data-cur="{{$item->currency->shortcut}}" data-map_id="{{$item->map_id}}" data-mekun="{{ $item->mekun }}" class="btn btn-warning btn-sm btnedit" style="width:50px;">កែ</a>
                               </td>
   ...
   @include('realestates.commissionlink_modal')
@endsection
@section('script')

    <script type="text/javascript">
        $('#h1_title').text('ពិនិត្យក្រុមកម្រៃជើងសារ');
        ...
        refreshtableFixHead(150);
        $(window).resize(function() {
            refreshtableFixHead(150);
        });
        $(document).ready(function () {
           $('#selpartner').select2({
            dropdownParent: $('#commissionlink_modal')
            });
  ...
  $(document).on('mousedown', '.modal-header', function(e) {
    ...
    $modal.addClass('no-transition'); // stop slide animation during drag
  });

  $(document).on('mousemove', function(e) {
    if (isDragging) {
      let $modal = $('.modal-dialog:visible');
      $modal.css({
  ...
  $(document).on('mouseup', function() {
    isDragging = false;
    $('.modal-dialog').removeClass('no-transition');
  });
            ...
            $(document).on('click','.btnedit',function(e){
                e.preventDefault();
                $('#commissionlink_modal').modal('show');

                $('#tid').val($(this).data('id'));
                $('#selpartner').val($(this).data('cid'));
                $('#amount').val(formatNumber(Math.abs($(this).data('amount'))));
                $('#cur').val($(this).data('cur'));
                $('#mapid').val($(this).data('map_id'));
             ...
             $(document).on('click','#btnupdate',function(e){
                e.preventDefault();
                $('body').addClass("wait");
                var formdata=new FormData(frm_commission_link);
                var url="{{ route('realestate.updatecommissionlink') }}";
                $.ajax({
                    async: true,
                    type: 'POST',
                    contentType: false,
                    ...
                    success: function (data) {
                        //console.log(data)
                        $('#commissionlink_modal').modal('hide');
