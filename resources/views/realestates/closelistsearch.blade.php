            ...
            <tbody id="body_closelist">
                ...
                @foreach ($transfers as $k => $tr)
                    ...
                    <tr class="@if(floatval($tr->count_pay + $tr->count_moredeposit + $tr->count_pay_over)<=0 && $tr->qtyleftday<0) c_red @endif">
                                ...
                                <ul class="dropdown-menu">
                                    @if (Auth::user()->role->name<>'Admin')
                                        @if (App\User::checkpermission(Auth::id(),'1.1.5'))
                                            <li>
                                                <a class="dropdown-item kh16-b" href="{{ route('realestate.showromloslist',['id'=>$tr->main_id,'customer_id'=>$tr->main_parrent_id,'customertype'=>$tr->main_customertype,'term'=>$tr->main_term,'rate'=>$tr->main_interest_rate,'startdate'=>$tr->main_startdate,'enddate'=>$tr->main_enddate,'curid'=>$tr->currency_id,'cursk'=>$tr->currency->sk,'curname'=>$tr->currency->shortcut,'payinmonth'=>$tr->main_payinmonth,'ispayromlos'=>1,'sendername'=>$tr->main_property]) }}" target="_blank"><i class="fa fa-building"></i> តារាងបង់រំលស់</a>
                                            </li>
                                    ...
                                    @else
                                        ...
                                        <li>
                                            <a class="dropdown-item kh16-b" href="{{ route('realestate.printromloslistforcustomer',['id'=>$tr->main_id,'customer_id'=>$tr->main_parrent_id,'customertype'=>$tr->main_customertype,'term'=>$tr->main_term,'rate'=>$tr->main_interest_rate,'startdate'=>$tr->main_startdate,'enddate'=>$tr->main_enddate,'curid'=>$tr->currency_id,'cursk'=>$tr->currency->sk,'curname'=>$tr->currency->shortcut,'payinmonth'=>$tr->main_payinmonth,'ispayromlos'=>1,'sendername'=>$tr->main_property]) }}" target="_blank"><i class="fa fa-print"></i> ព្រីនតារាងបង់ប្រាក់</a>
                                        </li>
                                        <li><a class="dropdown-item kh16-b btnpayment" style="" href="#" data-id="{{ $tr->main_id }}"><i class="fa fa-money"></i> កក់ប្រាក់បន្ថែម</a></li>
                                    @endif
                        ...
                        <td class="en16" title="{{$tr->id}}">{{ sprintf("%04d",$tr->main_id) . $tr->qtyleftday }}</td>
                        ...
                        <td class="en16">{{ $tr->nextpayment?date('d-m-Y',strtotime($tr->nextpayment)):'' }}</td>
                        ...
                        <td class="kh16-b" style="text-align:right;"  title="{{number_format($cuscharge,0,'.',' ')}}">{{ phpformatnumber($tr->punish+$tr->punish_over-$tr->punish_debt-$tr->punish_debt_over) .$tr->currency->sk }}</td>
                        <td class="kh16-b" style="text-align:right;"  title="{{$discount}}">{{ phpformatnumber($tr->punish_discount+$tr->punish_discount_over) .$tr->currency->sk }}</td>
                        ...
                        <td data-col="15" class="kh16-b selected" style="text-align:right;">
                            <a href="{{ route('realestate.showdeposit',['id'=>$tr->main_id,'customer_id'=>$tr->main_parrent_id,'customertype'=>$tr->main_customertype,'term'=>$tr->main_term,'rate'=>$tr->main_interest_rate,'startdate'=>$tr->main_startdate,'enddate'=>$tr->main_enddate,'curid'=>$tr->currency_id,'cursk'=>$tr->currency->sk,'curname'=>$tr->currency->shortcut,'payinmonth'=>$tr->main_payinmonth,'sendername'=>$tr->main_property]) }}" class="kh16-b " target="_blank" style="margin:0px;padding:2px;@if((floatval($tr->count_pay) + floatval($tr->count_moredeposit) + floatval($tr->count_pay_over))<=0 && $tr->qtyleftday<0) color:white; @endif">{{ phpformatnumber(abs($tr->deposited)) .$tr->currency->sk }}</a>
                        </td>
                        ...
                        <td class="kh16">{{ $tr->main_paymenttype==1?'បង់ផ្តាច់':'រំលស់' }}</td>
                        <td class="kh16" style="text-align:center;">{{ $tr->main_term?$tr->main_term . 'ខែ':'' }}</td>
                        <td class="kh16" style="text-align:center;">{{ $tr->main_interest_rate??0 }}%</td>
                        <td class="kh16" style="text-align:right;">{{ phpformatnumber($tr->main_payinmonth??0) . $tr->currency->sk}}</td>
                        <td class="kh16">{{ $tr->main_startdate?date('d-m-Y',strtotime($tr->main_startdate)):''}}</td>
                        <td class="kh16">{{ $tr->main_enddate?date('d-m-Y',strtotime($tr->main_enddate)):''}}</td>
                    </tr>
                @endforeach
            </tbody>
