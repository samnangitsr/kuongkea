...
<head>
    ...
    <title>Sold Property Print</title>
    ...
    <div id="invoice-pos">
        ...
        <div class="row" style="margin:20px 0px 0px 10px;">
            <div class="table-responsive">
                <table class="" style="width:100%;overflow:hidden">
                    <tr>
                        <td class="kh22-b" style="width:100%;text-align:center;padding:0px;"><abbr title="">{{ $title }}</abbr> </td>
                    </tr>
        ...
        <div class="row" style="margin:0px 10px 0px 20px;">
            ...
            <table id="tbl_header" class="table" style="table-layout:fixed;">
                <td style="width:75%">
                    <table>
                            <tr>
                                <td style="width:100px;font-size:16px;font-weight:bold;">លក់ជូន </td>
                            ...
                            @if($transfer->paymenttype==2)
                                <tr>
                                    <td style="width:80px;font-size:14px;font-weight:bold;">គិតពី</td>
                                    <td style="width:10px;">:</td>
                                    <td style="font-size:14px;font-weight:bold;">{{ date('d-m-Y',strtotime($transfer->startdate))}}</td>
                                </tr>
                                <tr>
                                    <td style="width:80px;font-size:14px;font-weight:bold;">ដល់</td>
                                    <td style="width:10px;">:</td>
                                    <td style="font-size:14px;font-weight:bold;">{{ date('d-m-Y',strtotime($transfer->enddate))}}</td>
                                </tr>
                                <tr>
                                    <td style="width:100px;font-size:16px;font-weight:bold;">បង់ប្រចាំខែ </td>
                                    <td style="width:10px;">:</td>
                                    <td style="font-size:16px;font-weight:bold;">{{ phpformatnumber($transfer->payinmonth) . $transfer->currency->sk }}</td>
                                </tr>
                            @endif
                    </table>
                </td>
                <td style="width:25%">
                    <table>
                        ...
                        <tr>
                            <td style="width:80px;font-size:14px;font-weight:bold;">អ្នកលក់</td>
                            <td style="width:10px;">:</td>
                            <td style="font-size:14px;font-weight:bold;">{{ $transfer->customer->name }}</td>
                        </tr>
                        <tr>
                            <td style="width:80px;font-size:14px;font-weight:bold;">ប្រភេទទូទាត់</td>
                            <td style="width:10px;">:</td>
                            <td style="font-size:14px;font-weight:bold;">{{ $transfer->paymenttype==2?'បង់រំលស់':'បង់ផ្តាច់' }}</td>
                        </tr>
                        @if($transfer->paymenttype==2)
                            <tr>
                                <td style="width:80px;font-size:14px;font-weight:bold;">រយះពេល</td>
                                <td style="width:10px;">:</td>
                                <td style="font-size:14px;font-weight:bold;">{{ $transfer->term }} ខែ អត្រា: {{ $transfer->interest_rate }} %</td>
                            </tr>
        ...
        <div class="row" style="margin:0px 10px 0px 20px;">
            <table id="tbldetail" class="table">
                ...
                <tbody>
                    @php
                        $countproperty=$saledetails->count();
                    @endphp
                   @foreach ($saledetails as $key => $sd)
                       <tr>
                        <td class="kh16" style="text-align:center;">{{ ++$key }}</td>
                        <td class="kh16">{{ $sd->property->name }}</td>
                        <td class="kh16">{{ $sd->property->group->type }}</td>
                        <td class="kh16">{!! $sd->property->size1 !!}</td>
                        <td class="en16" style="text-align:right;">{{ phpformatnumber($sd->price) . ' ' .  $sd->currency->shortcut }}</td>
                        <td class="en16" style="text-align:right;">{{ phpformatnumber($transfer->discount/$countproperty) .  $transfer->disc_by }}</td>

                       </tr>
                   @endforeach
                        <tr>
                                <td class="kh16-b" style="text-align:right;" colspan=5>
                                    សរុបទឹកប្រាក់
                                </td>
                                <td class="en16-b" style="text-align:right;">
                                    {{ phpformatnumber(abs($transfer->amount))  . ' ' . $transfer->currency->shortcut }}
                                </td>
                        </tr>
                        <tr>
                                <td class="kh16-b" style="text-align:right;" colspan=4>
                                    @if($transfer->deposit>0)
                                        @if($transfer->deposit_via_id=='cash')
                                            ទូទាត់តាម {{ $transfer->deposit_via . '(' . phpformatnumber($transfer->cash_amount) . $transfer->currency->sk . ')' }}
                                        @else
                                            ទូទាត់តាម {{ $transfer->deposit_via . '(' . phpformatnumber($transfer->bank_amount) . $transfer->currency->sk . ')' }}
                                        @endif
                                    @endif
                                </td>
                                ...
                                <td class="en16-b" style="text-align:right;">
                                {{ phpformatnumber(abs($transfer->deposit))  . ' ' . $transfer->currency->shortcut }}
                                </td>
                        </tr>
                        <tr>
                            <td class="kh16-b" style="text-align:right;" colspan=5>
                            ...
                            <td class="en16-b" style="text-align:right;">
                            {{ phpformatnumber(floatval(abs($transfer->amount))- floatval(abs($transfer->deposit)))  . ' ' . $transfer->currency->shortcut }}
                            </td>
                        </tr>
                </tbody>
            </table>
