...
<body>
    <div id="invoice-pos">
                ...
                <table class="table">
                    <tr>
                        <td style="padding:0px;border-style:none;">
                            <h1 class="kh16-b">ប្រតិបត្តិការណ៏ទាំងអស់របស់បុគ្គលិក {{ $title['user'] }}</h1>
                        </td>
                        <td style="padding:0px;border-style:none;text-align:right;">
                            <h1 class="kh16-b">{{ $title['date'] }}</h1>
                        </td>
                    </tr>
                </table>
            </div>
        ...
        <div class="row" style="margin:-15px 5px 0px 5px;">
          <div class="table-responsive">
              <table class="table table-bordered kh12-b tbl_usertransaction">
                  ...
                  <tbody>
                      ...
                      <tr>
                        ...
                        @endphp
                        <td class="en12-b" style="text-align:center;">{{ ++$key }}</td>
                        <td class="en12-b" style="text-align:center;">{{ $ut->tt }}</td>
                        <td class="en12-b" style="text-align:center;">{{ $ut->link_id}}</td>

                        <td class="kh12-b">
                            <a class="" href="#linkid{{ $ut->id }}" data-bs-toggle="collapse" title="{{ $ut->link_id }}" style=" text-decoration:underline;">{{ $ut->tranname }}</a>

                        </td>
                        <td class="amt @if($ut->gold>=0) blue @else red @endif @if($ut->issum==0) linethrough @endif">@if($ut->gold<>0) {{ phpformatnumber($ut->gold) . 'G' }} @endif </td>
                        <td class="amt @if($ut->usd>=0) blue @else red @endif @if($ut->issum==0) linethrough @endif">@if($ut->usd<>0){{ phpformatnumber($ut->usd) . '$' }} @endif</td>
                        <td class="amt @if($ut->thb>=0) blue @else red @endif @if($ut->issum==0) linethrough @endif">@if($ut->thb<>0) {{ phpformatnumber($ut->thb) . 'B' }} @endif</td>
                        <td class="amt @if($ut->khr>=0) blue @else red @endif @if($ut->issum==0) linethrough @endif">@if($ut->khr<>0){{ phpformatnumber($ut->khr) . 'R' }} @endif</td>
                        <td class="amt @if($ut->vnd>=0) blue @else red @endif @if($ut->issum==0) linethrough @endif">@if($ut->vnd<>0){{ phpformatnumber($ut->vnd) . 'V' }}@endif</td>
                        <td class="amt @if($ut->vnd>=0) blue @else red @endif @if($ut->issum==0) linethrough @endif">
                            @if($ut->fn<>'0')
                      ...
                      <tr style="background-color:aqua">
                          <td colspan=4 style="font-size:12px;font-weight:bold;">Total</td>
